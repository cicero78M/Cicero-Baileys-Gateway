...
**Cicero_V2** is an automation backend for monitoring social media, managing editorial workflows, and orchestrating WhatsApp messaging. The service ingests Instagram and TikTok metrics for multiple clients, tracks attendance, powers daily/weekly reporting, manages premium subscriptions, and now drives the Penmas editorial approval process. A single WhatsApp Gateway client is maintained to handle all messaging operations including operator interactions, user self-service menus, and gateway broadcasts—while OTP distribution has moved to instant email delivery.
